...
export default function OrderHistory() {
  ...
  // 🟢 Compute visible history at the top level to avoid Temporal Dead Zone errors
  const visibleHistory = useMemo(
    () => history.filter((o) => !clearedOrderIds.includes(o.orderId)),
    [history, clearedOrderIds]
  );
            ...
            {visibleHistory.length === 0 ? (
              ...
              <AnimatePresence mode="popLayout">
                {visibleHistory.map((order) => {
                  const keyId = order.orderId || order._id || "";
                  return (
                    <OrderHistoryItem
                      key={keyId}
                      order={order}
                      now={now}
                      isActiveMap={!!activeMaps[keyId]}
                      onToggleMap={() =>
                        setActiveMaps((p) => ({ ...p, [keyId]: !p[keyId] }))
                      }
