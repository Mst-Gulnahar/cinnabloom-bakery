...
export default function LoginPage() {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    try {
      ...
      // Send Google credential data to backend to issue an app JWT
      ...
      if (res.ok) {
        ...
        // Fallback to client-side auth state if backend route fails
      ...
      <div className="relative w-full max-w-md bg-[#FAF7F2]/90 backdrop-blur-md border border-[#E5E0D8] rounded-3xl p-6 sm:p-8 shadow-xl">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-2 bg-[#D0E3EA]/50 px-3.5 py-1 rounded-full border border-[#B0CEDB] mb-3">
            <LogIn size={13} className="text-[#D96B6B]" />
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#5A7A88]">Welcome Back</span>
          </div>
          <h1 className="text-2xl font-extrabold text-[#3E3835]">Log In</h1>
          <p className="text-xs text-[#7A736E] mt-1">Access your account to order fresh baked goodies ✨</p>
        </div>
        ...
        <p className="text-center text-xs text-[#7A736E] mt-5">
          ...
          <Link href="/signup" className="text-[#D96B6B] font-bold hover:underline">
            Sign Up
          </Link>
        </p>
      </div>
